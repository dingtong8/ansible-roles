...
function check_log_mysql() {
  ...
  log_failed_mysql=$(sudo cat `sudo find /var/log/ -type f -name 'mysql*' -newermt $(date +%Y-%m-01) ! -newermt $(date -d next-day +%Y-%m-%d)` | awk '/Access denied/{print $8}' | grep '@' | tr -d "'" | sort | uniq -c | sort -nr | awk '{print "'\''"$2"'\''"":""'\''"$1"'\''"","}')
  mysql_facts=$(
  cat <<EOF
  {
